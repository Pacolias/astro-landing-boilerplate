...
export const Hero: React.FC = () => {
  ...
  const renderLogos = (copy: number) =>
    logos.map((logo, index) => {
      ...
      return (
        <div
          key={`${copy}-${index}`}
          className="flex items-center gap-3 sm:gap-4 px-8 sm:px-12 shrink-0 text-slate-500 dark:text-slate-400 cursor-default select-none"
        >
          <Icon className="w-7 h-7 sm:w-10 sm:h-10 shrink-0" />
          <span className="text-base sm:text-2xl font-bold tracking-wider font-mono whitespace-nowrap">
            {logo.name}
          </span>
        </div>
      );
  ...
  return (
    ...
    >
      ...
      {/* Hero Main Content */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-14 sm:pt-20 lg:pt-28 pb-8 sm:pb-12 my-auto">
        <h1 className="text-3xl sm:text-5xl lg:text-6xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-4 sm:mb-6">
          Launch Your Next <br className="hidden sm:block" />
          <span className="text-blue-600 dark:text-blue-400">Great Project</span>
        </h1>

        <p className="text-base sm:text-xl text-slate-700 dark:text-slate-300 max-w-2xl mx-auto mb-6 sm:mb-10 leading-relaxed font-medium">
      ...
      <div className="relative z-10 pb-6 pt-2 sm:pb-10 sm:pt-4">
